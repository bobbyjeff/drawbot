...
rows = load_json("onlinerl_training_score_v1.json")
current_series = [(row["step"], row["current_avg_score"]) for row in rows]
baseline_series = [(row["step"], row["baseline_avg_score"]) for row in rows]
...
def interpolate_missing_values(series):
    ...
    for i, (_, y) in enumerate(filled):
        ...
        while prev_i >= 0 and filled[prev_i][1] is None:
            prev_i -= 1
        next_i = i + 1
        while next_i < len(filled) and filled[next_i][1] is None:
            next_i += 1
        if prev_i >= 0 and next_i < len(filled):
            x0, y0 = filled[prev_i]
            x1, y1 = filled[next_i]
            x = filled[i][0]
            t = 0 if x1 == x0 else (x - x0) / (x1 - x0)
            filled[i] = (x, y0 + t * (y1 - y0))
        elif prev_i >= 0:
            filled[i] = (filled[i][0], filled[prev_i][1])
        elif next_i < len(filled):
            filled[i] = (filled[i][0], filled[next_i][1])
    return filled
...
smoothed_series = [
    ("avg training score", PRIMARY, ema_series(current_series, 0.30)),
    ("baseline", BASELINE_GRAY, ema_series(interpolate_missing_values(baseline_series), 0.30)),
]
...
if show_labels:
    ...
    font("Helvetica-Bold")
    fontSize(34)
    text("Online RL training score v1", (chart_x, chart_y + chart_h + 28), align="left")
    font("Helvetica")
    fontSize(24)
    ...
    for y_tick in y_ticks:
        py = y_to_py(y_tick, y_min, y_max)
        text(f"{y_tick:.2f}", (grid_x, py + 8), align="left")
    ...
    with savedState():
        ...
        rotate(90)
        text("avg score", (0, 0), align="center")

    draw_legend(
        [("avg training score", PRIMARY), ("baseline", BASELINE_GRAY)],
        chart_x + chart_w - 500,
        chart_y + chart_h + 26,
    )
...
if shouldSave:
    base = os.path.join(EXPORTS, "chart-onlinerl-training-score-v1-" + currentTime.strftime("%Y%m%d-%H%M%S"))
    saveImage(base + ".png")
    saveImage(base + ".svg")
