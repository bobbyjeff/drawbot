...
animation_playback_speed = 9.0  # Faster playback without reducing per-segment motion samples.
base_motion_frames = 120
anim_kf3_extra_frame = 1  # Duplicate KF3 once so it lands like KF1, without a broader pause.
num_frames = (
    1
    if render_mode == "start"
    else base_motion_frames + (anim_kf3_extra_frame if render_mode == "anim" else 0)
)
frame_duration = (1.0 / _nominal_timeline_fps) / max(1e-9, animation_playback_speed)
# Export timing: GIF frame delays get clamped by many viewers, so speed must be enforced in ffmpeg.
export_retime_speed = 1.5
export_output_fps = 30
...
def timeline_uvwx(frame_index):
    """Four equal motion segments, with an optional single extra KF3 frame in full anim."""
    if render_mode != "anim":
    ...
    seg = max(1, base_motion_frames // 4)
    extra = anim_kf3_extra_frame
    b1 = seg
    b2 = 2 * seg
    b2x = b2 + extra
    b3 = b2x + seg
    b4 = b3 + seg
    if frame_index < b1:
        return ease01(frame_index / max(1, seg - 1)), 0.0, 0.0, 0.0
    if frame_index < b2:
        local = (frame_index - b1) / max(1, seg - 1)
        return 1.0, ease01(local), 0.0, 0.0
    if frame_index < b2x:
        return 1.0, 1.0, 0.0, 0.0
    if frame_index < b3:
        local = (frame_index - b2x) / max(1, seg - 1)
        return 1.0, 1.0, ease01(local), 0.0
    if frame_index < b4:
        local = (frame_index - b3) / max(1, seg - 1)
        return 1.0, 1.0, 1.0, ease01(local)
...
if shouldSave:
    ...
    raw_gif = os.path.join(EXPORTS, f"_raw-blog-warp-decode-anim-{ts}.gif")
    ...
    def make_scaled_gif(width):
        out = os.path.join(EXPORTS, f"blog-warp-decode-anim-{ts}-{width}.gif")
        pal = os.path.join(EXPORTS, f"_palette-warp-decode-anim-{ts}-{width}.png")
        subprocess.run(
            [
                FFMPEG,
                "-y",
                ...
                src_gif,
                "-vf",
                f"scale={width}:-1:flags=lanczos,palettegen=stats_mode=diff:max_colors=64",
                pal,
            ],
            check=True,
        )
        subprocess.run(
            [
                ...
                pal,
                "-lavfi",
                f"scale={width}:-1:flags=lanczos[x];[x][1:v]paletteuse=dither=bayer:bayer_scale=5",
                out,
            ],
            check=True,
        )
    ...
    full_palette = os.path.join(EXPORTS, f"_palette-full-warp-decode-anim-{ts}.png")
    subprocess.run(
        [
            FFMPEG,
            "-y",
            ...
            raw_gif,
            "-vf",
            f"setpts=PTS/{export_retime_speed},fps={export_output_fps},palettegen=stats_mode=diff:max_colors=64",
            full_palette,
        ],
        check=True,
    )
    subprocess.run(
        [
            ...
            full_palette,
            "-lavfi",
            f"setpts=PTS/{export_retime_speed},fps={export_output_fps}[x];[x][1:v]paletteuse=dither=bayer:bayer_scale=5",
            src_gif,
        ],
        check=True,
    )
    os.remove(full_palette)
    os.remove(raw_gif)

    out_gif = make_scaled_gif(1200)
    out_gif_800 = make_scaled_gif(800)
    out_gif_600 = make_scaled_gif(600)
    ...
    print(
        f"Saved: {src_gif}, {out_gif}, {out_gif_800}, {out_gif_600}, {src_mp4}, {out_mp4}"
    )
